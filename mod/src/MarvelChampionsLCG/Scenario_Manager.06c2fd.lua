...
function setMode(params)
    ...
    if (params.mode == "standard") then
        currentScenario.expertSet = nil
    end
...
function hideSelectors()
    ...
    layoutManager.call("hideSelectors", {
        itemType = "scenario"
    })
end
...
function showSelectors()
    ...
    local layoutManager = getObjectFromGUID(Global.getVar("GUID_LAYOUT_MANAGER"))
    layoutManager.call("showSelectors", {
        itemType = "scenario"
    })
    layoutManager.call("highlightSelectedSelectorTile", {
        itemType = "scenario",
        itemKey = selectedScenarioKey
    })
end

function placeScenario(scenarioKey, mode)
    if not confirmNoScenarioIsPlaced() then
        return
    end
...
function setInitialFirstPlayer()
    ...
    heroManager.call("setFirstPlayer", {
        playerColor = currentScenario.firstPlayer
    })
end

function placeNotes()
    if (currentScenario.notes == nil) then
        return
    end
...
function modularSetsAreValid(params)
    ...
    if (params and params.postMessage and not modularSetsAreValid) then
        ...
        broadcastToAll("Please select at least " .. additionalEncounterSets .. " more encounter set" .. plural .. ".",
            {1, 1, 1})
    end
...
function modeAndStandardEncounterSetsAreValid(params)
    ...
    local expertSetIsValid = currentScenario.expertSet ~= nil or currentScenario.mode ~= "expert" or
                                 not scenarioUsesStandardEncounterSets
...
function getRequiredEncounterSetCount()
    ...
    if (currentScenario.modularSets == nil) then
        return 0
    end
...
function configureVillainHpCounter(params)
    ...
    Wait.frames(function()
        counter.call("setVillainKey", {
            villainKey = villainKey
        })

        if (hitPoints > 0) then
            counter.call("setValue", {
                value = hitPoints
            })
        end
...
function addObligationCardsToEncounterDeck(deck)
    ...
    if (not useHeroObligations) then
        return
    end
...
function setUpCards()
    if (currentScenario.cards == nil) then
        return
    end
...
function setUpCounters(heroCount)
    if (currentScenario.counters == nil) then
        return
    end
...
function placeVillainCard(params)
    local villain = params.villain
    local cardId = params.cardId
    local name = params.name
    local position = params.position or defaults.villainDeck.position
    local rotation = params.rotation or defaults.villainDeck
...
function placeDeck(deck)
    ...
    local spawnedDeck = Global.call("spawnDeck", {
        cards = deck.cards,
        ...
        function moveLinkedCardCoroutine()
            ...
            Wait.frames(function()
                Global.call("nameDeck", {
                    deckPosition = linkedCardPosition,
                    name = "Linked Cards"
                })
            end, 120)
            ...
            spawnObject({
                ...
                rotation = {90, 0, 0},
                callback_function = function(spawned_object)
                    spawned_object.TextTool.setValue("Linked\nCards")
...
function placeThreatCounter(counter, threat)
    ...
    local threatCounter = threatCounterBag.takeObject({
        position = threatCounterPosition,
        smooth = false
    })
...
function placeGeneralCounter(counter)
    local counterPosition = counter.position
    local counterRotation = counter.rotation or {180, 0, 180}
    local counterScale = counter.scale
    local counterName = counter.name or ""
    local counterBag = getObjectFromGUID(Global.getVar("GUID_LARGE_GENERAL_COUNTER_BAG"))
    local generalCounter = counterBag.takeObject({
        position = counterPosition,
        smooth = false
    })
    local value = counter.value or 0
    local tags = counter.tags or {}
    local locked = counter.locked
    if locked == nil then
        locked = true
    end
    ...
    Wait.frames(function()
        ...
        for _, tag in pairs(tags) do
            generalCounter.addTag(tag)
        end

        generalCounter.call("setValue", {
            value = value
        })
    end, 1)
end
...
function addItemToManifest(key, item)
    if (not currentScenario.manifest) then
...
function placeModularSets(scenario)
    if (scenario.modularSets == nil) then
        return
    end
    ...
    for key, value in pairs(scenario.modularSets) do
        local modularSet = modularSetManager.call("getModularSet", {
            modularSetKey = key
        })
    ...
    if (recommendedSetCount > 0) then
        ...
        message = message .. "Add " .. recommendedSetCount .. modular .. "set" .. plural .. " (recommended: " ..
                      recommendedSetList:sub(1, -3) .. ")."
    end
...
function findCardsAtPosition3(obj)
    ...
    for _, obj in ipairs(objList) do
        ...
        if (hitObject.tag == "Card") then
            local aspect = Global.call("getCardProperty", {
                card = hitObject,
                property = "aspect"
            })
...
function getNextVillainStage(villainKey)
    ...
    if (nextStage.showAdvanceButton == nil) then
        ...
        local showAdvanceButton = nextStageNumber == "1" or (nextStageNumber == "2" and mode == "expert")
...
function placeSchemeStage(schemeKey, stage, heroCount)
    ...
    if (currentScenario.fullyScripted) then
...
function getHpCounterForVillain(params)
    local villain = currentScenario.villains[params.villainKey]

    if (villain == nil or villain.hpCounter == nil or villain.hpCounter.guid == nil) then
        return nil
    end
